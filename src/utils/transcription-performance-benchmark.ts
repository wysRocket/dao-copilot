...
export class TranscriptionPerformanceBenchmark {
  ...
  markWebSocketConnected(): void {
    ...
    console.log(
      '🔌 WebSocket connected in:',
      (timestamp - this.metrics.websocketConnection.connectionStart).toFixed(2),
      'ms'
    )
  }
  ...
  markFirstResponseReceived(): void {
    ...
    performance.measure(
      'websocket-total-latency',
      'websocket-connection-start',
      'websocket-first-response'
    )

    console.log(
      '🌐 WebSocket first response in:',
      this.metrics.websocketConnection.latency.toFixed(2),
      'ms'
    )
  }
  ...
  markAudioProcessingComplete(): void {
    ...
    this.metrics.audioProcessing.latency = timestamp - this.metrics.audioProcessing.startTime
    ...
    console.log(
      '🔊 Audio processing complete in:',
      this.metrics.audioProcessing.latency.toFixed(2),
      'ms'
    )
  ...
  markRenderComplete(): void {
    ...
    performance.measure(
      'transcription-display-duration',
      'transcription-received',
      'transcription-render-complete'
    )

    console.log(
      '🎨 Transcription rendered in:',
      this.metrics.transcriptionDisplay.latency.toFixed(2),
      'ms'
    )
  }
  ...
  completeBenchmark(): PerformanceMetrics {
    ...
    console.log(
      '🏁 Total transcription latency:',
      this.metrics.endToEnd.totalLatency.toFixed(2),
      'ms'
    )
    ...
    if (this.metrics.websocketConnection) {
      ...
      console.log(
        ...
        (
          ...
          this.metrics.websocketConnection.connectionStart
        ).toFixed(2),
        'ms'
      )
      console.log(
  ...
  compareWithYouTube(): void {
    ...
    if (this.metrics.audioCapture) {
      ...
      console.log(
        `🎵 Audio Capture: ${this.metrics.audioCapture.latency.toFixed(2)}ms vs ${youtubeLatency.audioCapture}ms (${diff > 0 ? '+' : ''}${diff.toFixed(2)}ms)`
      )
    }
    ...
    if (this.metrics.websocketConnection) {
      ...
      console.log(
        `🔌 WebSocket: ${this.metrics.websocketConnection.latency.toFixed(2)}ms vs ${youtubeLatency.websocket}ms (${diff > 0 ? '+' : ''}${diff.toFixed(2)}ms)`
      )
    }
    ...
    if (this.metrics.transcriptionDisplay) {
      ...
      console.log(
        `🎨 Display: ${this.metrics.transcriptionDisplay.latency.toFixed(2)}ms vs ${youtubeLatency.display}ms (${diff > 0 ? '+' : ''}${diff.toFixed(2)}ms)`
      )
    }

    const totalDiff = this.metrics.endToEnd.totalLatency - youtubeLatency.total
    console.log(
      `🏁 Total: ${this.metrics.endToEnd.totalLatency.toFixed(2)}ms vs ${youtubeLatency.total}ms (${totalDiff > 0 ? '+' : ''}${totalDiff.toFixed(2)}ms)`
    )

    if (totalDiff > 0) {
      console.log(
        `🚀 Target improvement: ${((totalDiff / this.metrics.endToEnd.totalLatency) * 100).toFixed(1)}% faster needed`
      )
    } else {
      console.log(
        `✅ Performance is ${((-totalDiff / youtubeLatency.total) * 100).toFixed(1)}% better than YouTube!`
      )
    }
