...
export class WebSocketConnectionEstablisher extends EventEmitter {
  ...
  constructor(config: ConnectionConfig, errorHandler?: GeminiErrorHandler) {
    ...
    this.config = {
      endpoint: 'wss://generativelanguage.googleapis.com/ws/google.ai.generativelanguage.v1beta.GenerativeService.LiveStreaming',
   ...
   */
  private async validateConnection(websocket: WebSocket, metrics: ConnectionMetrics): Promise<void> {
    ...
    if (this.config.validation?.requiredProtocols) {
      ...
      if (!this.config.validation.requiredProtocols.includes(actualProtocol)) {
        throw new Error(`Protocol mismatch. Expected: ${this.config.validation.requiredProtocols.join(', ')}, Got: ${actualProtocol}`)
      }
    }
  ...
  async cleanup(): Promise<void> {
    ...
    await Promise.all(
      connectionIds.map(id => this.closeConnection(id, 1000, 'Service shutdown'))
    )
