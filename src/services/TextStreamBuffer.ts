 ...
 */
export type TextBufferListener = (text: string, isPartial: boolean, metadata?: TextChunk['metadata']) => void;

/**
 * Event types for the text buffer
 */
export type TextBufferEventType = 'textUpdate' | 'chunkAdded' | 'chunkRemoved' | 'bufferCleared' | 'correctionDetected';
...
export class TextStreamBuffer {
   ...
   */
  public addChunk(text: string, isPartial: boolean = false, metadata?: TextChunk['metadata']): string {
    ...
    if (this.config.enableCorrectionDetection && this.buffer.length > 0) {
      ...
      if (this.detectCorrection(lastChunk.text, text)) {
        ...
        this.emit('correctionDetected', text, isPartial, { ...metadata, correctedFrom: lastChunk.text });
      }
    }
  ...
  private emit(eventType: TextBufferEventType, text: string, isPartial: boolean, metadata?: TextChunk['metadata']): void {
    const listeners = this.listeners.get(eventType) || [];
