...
import { migrateLegacyEnvironment, isLegacyUsagePattern } from '../services/transcription-compatibility'
...
export const DEFAULT_CONFIG: Partial<GeminiWebSocketConfig> = {
  ...
  websocketUrl: 'wss://generativelanguage.googleapis.com/ws/v1beta/models/gemini-2.5-flash-preview-05-20:streamGenerateContent',
...
export function validateConfig(config: GeminiWebSocketConfig): ConfigValidationResult {
  ...
  if (config.websocketEnabled && !config.proxyFallbackEnabled) {
    recommendations.push('Consider enabling proxy fallback for environments with restricted network access.')
  }
 ...
 */
export function getValidatedConfig(): { config: GeminiWebSocketConfig; validation: ConfigValidationResult } {
