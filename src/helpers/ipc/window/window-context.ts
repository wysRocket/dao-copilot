    ...
    if (!globalWindow.electronWindow) {
      contextBridge.exposeInMainWorld('electronWindow', {
        ...
        showWindow: (windowId: string) => 
          ipcRenderer.invoke(WINDOW_SHOW_CHANNEL, windowId),
        hideWindow: (windowId: string) => 
          ipcRenderer.invoke(WINDOW_HIDE_CHANNEL, windowId),
        focusWindow: (windowId: string) => 
          ipcRenderer.invoke(WINDOW_FOCUS_CHANNEL, windowId),
        getAllWindows: () => 
          ipcRenderer.invoke(WINDOW_GET_ALL_CHANNEL),
        getWindowInfo: (windowId?: string) => 
          ipcRenderer.invoke(WINDOW_GET_INFO_CHANNEL, windowId),
        ...
        onInterWindowMessage: (callback: (channel: string, ...args: any[]) => void) => {
          const listener = (_event: any, channel: string, ...args: any[]) => callback(channel, ...args);
