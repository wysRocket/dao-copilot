...
export const useTextCorrection = (
   ...
   */
  const detectCorrections = useCallback((oldText: string, newText: string): (TextDiff | WordDiff)[] => {
   ...
   */
  const applyCorrectionsToText = useCallback((baseText: string, corrections: ActiveCorrection[]): string => {
  ...
  const updateAnimations = useCallback(() => {
    ...
    setActiveCorrections(prevCorrections => {
      return prevCorrections.map(correction => {
  ...
  }, [activeCorrections.length, isPaused, finalConfig.correctionDuration, finalConfig.highlightDuration]);

  /**
   * Start correction animation
   */
  const startCorrection = useCallback((diff: TextDiff | WordDiff) => {
    ...
    setStats(prev => ({
      ...
      averageLatency: (prev.averageLatency * prev.totalCorrections + (Date.now() - startTimeRef.current)) / (prev.totalCorrections + 1)
    }));
  ...
  const processQueue = useCallback(() => {
    ...
    processNext();
  }, [correctionQueue.length, isProcessingCorrections, isPaused, startCorrection, finalConfig.correctionDelay]);
   ...
   */
  const animateCorrection = useCallback((diff: TextDiff | WordDiff) => {
