...
export const useAccessibility = (options: UseAccessibilityOptions = {}): UseAccessibilityReturn => {
  ...
  const [preferences, setPreferences] = useState<AccessibilityPreferences>(() => {
    const detected = autoDetect ? detectAccessibilityPreferences() : {
      reducedMotion: false,
      highContrast: false,
      forceFocus: false,
  ...
  // Announce function
  const announce = useCallback((text: string, priority: AnnouncementPriority = 'medium') => {
    if (preferences.announceChanges && liveRegionManagerRef.current) {
  ...
  // Focus function
  const setFocus = useCallback((element: HTMLElement, options?: FocusOptions): boolean => {
    if (enableFocusManagement && focusManagerRef.current) {
  ...
  const createButtonHandler = useCallback((onClick: () => void) => {
    return {
      onClick,
  ...
  const createNavigationHandlers = useCallback((navigationOptions: {
    onUp?: () => void;
    onDown?: () => void;
    onLeft?: () => void;
    onRight?: () => void;
