...
export const useOptimizedTranscriptionState = (
  ...
  const derivedState = useMemo(() => {
    ...
    const totalWords = segments.reduce((count, segment) => 
      count + segment.text.split(/\s+/).filter(word => word.length > 0).length, 0
    );
   ...
   */
  const updateState = useCallback((updater: (draft: TranscriptionState) => void) => {
   ...
   */
  const updateCurrentSegment = useCallback((text: string, confidence = 0.8) => {
    updateState(draft => {
  ...
  return {
    ...
    getPerformanceMetrics: () => ({
      ...
      averageUpdatesPerSecond: updateCountRef.current / ((Date.now() - sessionStartRef.current) / 1000),
      memoryUsage: state.segments.length,
      isPending
    }),

    // Advanced utilities
    exportTranscripts: () => state.segments.map(segment => ({
      text: segment.text,
