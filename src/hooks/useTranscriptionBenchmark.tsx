...
} from '../utils/transcription-performance-benchmark'
  ...
  const getRecommendations = useCallback((): string[] => {
    ...
    if (averages.audioCapture > 100) {
      recommendations.push(
        'Consider optimizing audio capture initialization - use pre-warmed AudioContext'
      )
    }
    ...
    if (averages.processing > 300) {
      recommendations.push(
        'Audio processing bottleneck detected - consider WebAssembly or WebWorkers'
      )
    }

    if (averages.display > 50) {
      recommendations.push(
        'UI rendering is slow - implement virtualization or React.memo optimizations'
      )
    }

    if (averages.total > 1000) {
      recommendations.push(
        'Overall latency is high - prioritize parallel processing and pre-warming'
      )
    }
  ...
  if (compact) {
    return (
      <div className="benchmark-display-compact">
        ...
        {current.endToEnd && (
          <div className="total-latency">Total: {current.endToEnd.totalLatency.toFixed(0)}ms</div>
        )}
      </div>
    )
  }
  ...
  return (
    <div className="benchmark-display">
      ...
      <div className="current-metrics">
        ...
        {current.endToEnd && (
          <div>
            <strong>Total: {current.endToEnd.totalLatency.toFixed(2)}ms</strong>
          </div>
        )}
      </div>
      ...
      {history.length > 0 && (
        <div className="averages">
          ...
          <div>
            <strong>Total: {averages.total.toFixed(2)}ms</strong>
          </div>
        </div>
      )}
