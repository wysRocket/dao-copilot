...
export const useTranscriptionState = (
  ...
  useEffect(() => {
    ...
    const unsubscribe = stateManager.subscribe(
      (type: StateChangeType, newState: TranscriptionState) => {
        setState(newState)
      }
    )
  ...
  const setRecordingState = useCallback(
    (isRecording: boolean, recordingTime: number = 0, status: string = 'Ready') => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onStateChange((type: StateChangeType, newState: TranscriptionState) => {
      ...
      const electronWindow = (
        window as Window & {
          electronWindow?: {
            broadcast: (channel: string, type: StateChangeType, state: TranscriptionState) => void
          }
        }
      ).electronWindow
    ...
    const electronWindow = (
      window as Window & {
        electronWindow?: {onInterWindowMessage: (handler: typeof handleStateSync) => () => void}
      }
    ).electronWindow
    if (
      typeof window !== 'undefined' &&
      electronWindow &&
      'onInterWindowMessage' in electronWindow
    ) {
