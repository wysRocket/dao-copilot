...
interface MultiWindowContextType {
  ...
  updateSharedState: <K extends keyof SharedState>(
    key: K,
    value: SharedState[K],
  ) => void;
  syncState: () => void;
  subscribeToStateChanges: (
    callback: (newState: SharedState) => void,
  ) => () => void;
  broadcastStateChange: <K extends keyof SharedState>(
    key: K,
    value: SharedState[K],
  ) => void;
}
...
export const useMultiWindowContext = () => {
  ...
  if (!context) {
    throw new Error(
      'useMultiWindowContext must be used within a MultiWindowProvider',
    );
  }
  return context;
};
...
export const MultiWindowProvider: React.FC<MultiWindowProviderProps> = ({
  children,
}) => {
