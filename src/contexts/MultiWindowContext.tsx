...
export const MultiWindowProvider: React.FC<MultiWindowProviderProps> = ({ children }) => {
  ...
  const [stateChangeCallbacks, setStateChangeCallbacks] = useState<Array<(newState: SharedState) => void>>([]);
  ...
  useEffect(() => {
    const removeListener = window.electronWindow?.onInterWindowMessage?.((channel: string, ...args: any[]) => {
  ...
  const updateSharedState = useCallback(<K extends keyof SharedState>(key: K, value: SharedState[K]) => {
    setSharedState(prev => {
  ...
  const broadcastStateChange = useCallback(<K extends keyof SharedState>(key: K, value: SharedState[K]) => {
    // Update local state
  ...
  const updateSettings = useCallback((newSettings: Partial<SharedState['settings']>) => {
  ...
  const setTheme = useCallback((theme: SharedState['theme']) => {
  ...
  return (
    <MultiWindowContext.Provider value={contextValue}>
      {children}
