...
class MockWebSocket implements Partial<WebSocket> {
  ...
  // Implement addEventListener to match WebSocket interface
  ...
  // Implement removeEventListener to match WebSocket interface
  ...
  private emitToListeners(type: string, event: Event): void {
    const listeners = this.listeners.get(type) || []
  ...
  simulateOpen() {
    ...
    this.emitToListeners('open', new Event('open'))
  }
  ...
  simulateClose(code = 1000, reason = 'Normal closure') {
    ...
    this.emitToListeners('close', closeEvent)
  }
  ...
  simulateMessage(data: unknown) {
    ...
    this.emitToListeners('message', messageEvent)
  }
...
describe('ConnectionMonitor', () => {
  ...
  describe('Initialization', () => {
    it('should initialize with default configuration', () => {
      ...
      expect(defaultMonitor.getQuality()).toBe(1.0)
    })
    ...
    it('should initialize metrics correctly', () => {
      ...
      expect(metrics.connectionQuality).toBe(1.0)
  ...
  describe('Heartbeat Mechanism', () => {
    ...
    it('should reset consecutive timeouts after successful pong', () => {
      ...
      // Just verify that pong handling works correctly after timeouts
      // Send a ping and immediately respond with pong
      vi.advanceTimersByTime(1000) // Send first ping

      // Get the ping message and respond
      const calls = (mockWebSocket.send as Mock).mock.calls
      const pingMessage = calls[calls.length - 1][0]
      const pingData = JSON.parse(pingMessage)
      mockWebSocket.simulatePong(pingData.ping)

      // Verify that the pong was handled successfully
      expect(heartbeatSuccessSpy).toHaveBeenCalledTimes(1)
      expect(heartbeatSuccessSpy).toHaveBeenCalledWith(
        expect.objectContaining({
          latency: expect.any(Number)
        })
