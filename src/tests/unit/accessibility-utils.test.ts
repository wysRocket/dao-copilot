...
describe('Accessibility Utils', () => {
  beforeEach(() => {
    ...
    // Mock window.matchMedia
    (window as WindowWithMediaQuery).matchMedia = vi.fn().mockImplementation(() => mockMatchMedia(false));
  });
  ...
  describe('AriaLiveRegionManager', () => {
    ...
    it('should announce text to appropriate regions', async () => {
      const spy = vi.spyOn(manager as any, 'processQueue');
  ...
  describe('KeyboardUtils', () => {
    ...
    it('should prevent default for specified keys', () => {
      const enterEvent = new KeyboardEvent('keydown', { key: 'Enter' });
      const spyPreventDefault = vi.spyOn(enterEvent, 'preventDefault');
