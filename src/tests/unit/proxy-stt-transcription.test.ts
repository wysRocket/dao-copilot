...
describe('Proxy STT Transcription Service', () => {
  ...
  describe('transcribeAudioViaProxy (legacy)', () => {
    ...
    it('should throw error when API key is missing', async () => {
      await expect(
        transcribeAudioViaProxy(mockAudioData, {})
    ...
    it('should handle proxy server errors', async () => {
      ...
      await expect(
        transcribeAudioViaProxy(mockAudioData, { apiKey: mockApiKey })
      ).rejects.toThrow('Batch proxy transcription failed: 500 Internal Server Error')
    })
  })
  ...
  describe('Configuration validation', () => {
    ...
    it('should warn about WebSocket configuration mismatch', () => {
      ...
      expect(validation.warnings).toContain(
        'WebSocket mode requested but WebSocket is not enabled'
      )
    })
  })
