...
import GeminiToolCallIntegrationService, { type GeminiToolCallIntegrationConfig } from '../gemini-tool-call-integration.js';
import { ResponseModality, ConnectionState } from '../gemini-live-websocket.js';

// Mock dependencies
jest.mock('../tool-enabled-gemini-websocket.js');
jest.mock('../tool-call-handler.js');
jest.mock('../question-detector.js');
jest.mock('../transcription-question-pipeline.js');
jest.mock('../multi-part-question-processor.js');
jest.mock('../gemini-logger.js');
...
describe('Gemini Tool Call Integration - End to End', () => {
  ...
  it('should complete a full question-answer-search workflow', async () => {
    ...
    const mockSearchResult = {
      success: true,
      results: [
        { title: 'AI Definition', snippet: 'Artificial Intelligence...', link: 'https://example.com/ai' }
      ],
    ...
    const events: any[] = [];
    ['questionDetected', 'toolCallRequested', 'searchStarted', 'searchCompleted', 'toolCallCompleted']
      .forEach(event => {
        integrationService.on(event, (data) => events.push({ event, data }));
      });
