      ...
      <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-gray-800 rounded-lg p-6">
              ...
              {Object.keys(newSystemMetrics).length > 0 && (
                <div className="mt-4 p-3 bg-green-900/20 rounded">
                  <div className="text-sm text-green-400 font-medium mb-2">Performance Metrics</div>
                  <div className="grid grid-cols-2 gap-2 text-xs text-gray-300">
                    <div>Latency: <span className="text-green-400">{newSystemMetrics.latency?.toFixed(2)}ms</span></div>
                    <div>Rate: <span className="text-blue-400">{newSystemMetrics.messagesPerSecond?.toFixed(1)}/s</span></div>
                    <div>Total: <span className="text-yellow-400">{newSystemMetrics.totalMessages}</span></div>
                    <div>Errors: <span className="text-red-400">{newSystemMetrics.errors || 0}</span></div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="bg-gray-800 rounded-lg p-6">
              ...
              <div className="min-h-[300px] flex items-center justify-center">
                <div className="text-center text-gray-400">
                  ...
                  <div className="text-lg font-medium mb-2">Simulated Delay</div>
                  <div className="text-sm">
                    This represents the old system with<br />
                    3-5 second transcription delays
                  </div>
                  <div className="mt-4 p-3 bg-red-900/20 rounded">
                    <div className="text-sm text-red-400 font-medium mb-2">Typical Metrics</div>
                    <div className="grid grid-cols-2 gap-2 text-xs text-gray-300">
                      <div>Latency: <span className="text-red-400">3000-5000ms</span></div>
                      <div>Rate: <span className="text-red-400">0.2-0.5/s</span></div>
                      <div>Buffering: <span className="text-red-400">High</span></div>
                      <div>User Experience: <span className="text-red-400">Poor</span></div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-gray-800 rounded-lg p-6">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-700">
                    ...
                    <th className="text-left py-2 text-gray-300">Avg Latency</th>
                    <th className="text-left py-2 text-gray-300">Max Latency</th>
                    <th className="text-left py-2 text-gray-300">Min Latency</th>
                    <th className="text-left py-2 text-gray-300">Rate</th>
                    <th className="text-left py-2 text-gray-300">Render Time</th>
                  </tr>
                </thead>
                <tbody>
                  {benchmark.map((result, index) => (
                    <tr key={index} className={cn(
                      ...
                      result.component.includes('New System') ? 'bg-green-900/10' : 'bg-red-900/10'
                    )}>
                      <td className="py-2 font-medium">
                        <span className={cn(
                          result.component.includes('New System') ? 'text-green-400' : 'text-red-400'
                        )}>
                          {result.component}
                        </span>
                      </td>
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-green-900/20 p-4 rounded">
                <div className="text-green-400 font-medium">🚀 Performance Gain</div>
                <div className="text-2xl font-bold text-white">95%+</div>
                <div className="text-sm text-gray-300">Latency reduction</div>
              </div>
              <div className="bg-blue-900/20 p-4 rounded">
                <div className="text-blue-400 font-medium">⚡ Speed Improvement</div>
              ...
              <div className="bg-purple-900/20 p-4 rounded">
                <div className="text-purple-400 font-medium">📈 Message Rate</div>
        ...
        <div className="bg-gray-800 rounded-lg p-6 mt-6">
          ...
          <div className="space-y-2 text-gray-300">
            ...
            <p>2. <strong>Test:</strong> Click "Test" to simulate transcription updates</p>
            <p>3. <strong>Benchmark:</strong> Run different test types to measure performance</p>
            <p>4. <strong>Compare:</strong> Toggle comparison view to see old vs new system</p>
            <p>5. <strong>Monitor:</strong> Watch real-time metrics in the performance overlay</p>
          </div>
        </div>
      </div>
    </div>
