...
const LiveStreamingArea: React.FC<LiveStreamingAreaProps> = ({
  ...
  const [animationPhase, setAnimationPhase] = useState<'entering' | 'active' | 'completing' | 'exiting'>('entering')
  ...
  return (
    <div
      ...
      className={cn(
        ...
        {
          'opacity-0 transform scale-95': !isExpanded && animationPhase === 'entering',
          'opacity-100 transform scale-100': isExpanded && animationPhase === 'active',
    ...
    >
      <GlassBox 
        ...
        className={cn(
          ...
          'border transition-all duration-500 focus-visible-enhanced',
          {
            'streaming-header-mobile': true, // Responsive class
            'border-blue-400/40 shadow-blue-400/20 shadow-lg': animationPhase === 'active' && animate,
            'border-green-400/40 shadow-green-400/20 shadow-lg': animationPhase === 'completing' && animate,
        ...
        style={{
          ...
          background: animationPhase === 'completing' 
      ...
      >
        ...
        <div className={cn(
          ...
          "md:flex-row sm:flex-col sm:items-start sm:space-y-2 sm:space-x-0",
          ...
          <div className={cn(
            ...
            {showConfidenceScore && confidence !== undefined && confidence > 0 && (
              <div className={cn(
                ...
                "streaming-confidence-mobile"
              )}>
                <div 
                  className={cn(
                    "text-xs px-2 py-1 rounded backdrop-blur-sm border transition-all duration-300",
                    "glass-button focus-visible-enhanced"
                  )}
                  style={{
                    backgroundColor: confidence >= 0.8 ? 'rgba(16, 185, 129, 0.1)' : 
                                   confidence >= 0.6 ? 'rgba(59, 130, 246, 0.1)' :
                                   'rgba(239, 68, 68, 0.1)',
                    borderColor: confidence >= 0.8 ? 'rgba(16, 185, 129, 0.3)' : 
                                confidence >= 0.6 ? 'rgba(59, 130, 246, 0.3)' :
                                'rgba(239, 68, 68, 0.3)',
                    color: confidence >= 0.8 ? '#10b981' : 
                          confidence >= 0.6 ? '#3b82f6' :
                          '#ef4444'
                  }}
            ...
            {streamingText && streamingText.length > 100 && animationPhase === 'active' && (
              <div 
                className="text-xs px-2 py-1 rounded backdrop-blur-sm border animate-pulse"
                style={{
            ...
            {onClearStreaming && (
              <button
                ...
                className={cn(
                  "p-1 rounded glass-button focus-visible-enhanced",
        ...
        <div className={cn(
          ...
          <div 
            className={cn(
              "min-h-[60px] flex items-start justify-start transition-all duration-500",
        ...
        {animate && animationPhase === 'active' && (
          <div className="absolute bottom-0 left-0 right-0 h-0.5 overflow-hidden rounded-b-[16px]">
            <div className="h-full streaming-progress-bar" />
          </div>
        )}
      </GlassBox>
    </div>
