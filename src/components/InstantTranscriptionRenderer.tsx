...
const AnimatedCursor = memo(() => (
  <span
    className="ml-1 inline-block h-4 w-1 animate-pulse bg-blue-400"
    style={{
...
const TextChunkRenderer = memo<{
  ...
  return (
    <div style={style} className="flex items-start">
      <span
        className={cn('text-sm leading-relaxed transition-opacity duration-100', {
  ...
  ({
    ...
    return (
      ...
      >
        {/* Performance overlay */}
        {showMetrics && (
          <div className="absolute right-0 top-0 z-10 rounded bg-black/50 p-1 text-xs text-green-400">
        ...
        {!minimal && (
          <div className="mb-2 flex items-center justify-between text-xs text-gray-400">
            <div className="flex items-center space-x-2">
              <div
                className={cn('h-2 w-2 rounded-full transition-colors duration-200', {
                  'animate-pulse bg-green-400': isActive,
            ...
            {shouldVirtualize && (
              <div className="text-gray-500">{chunks.length} chunks | Virtualized</div>
            )}
          </div>
        )}

        {/* Main content */}
        <div className="max-h-96 min-h-[60px] overflow-hidden">{contentRenderer}</div>
 ...
 */
export const UltraOptimizedTranscriptionRenderer = memo<InstantTranscriptionProps>(props => {
