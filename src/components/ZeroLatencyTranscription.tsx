...
import { UltraFastWebSocketManager, getUltraFastWebSocketManager } from '../services/UltraFastWebSocketManager'
import { InstantTranscriptionRenderer, UltraOptimizedTranscriptionRenderer } from './InstantTranscriptionRenderer'
...
function useZeroLatencyTranscription(websocketManager: UltraFastWebSocketManager) {
  ...
  const updateTranscription = useCallback((text: string, isPartial: boolean, metadata: any = {}) => {
...
const PerformanceMetrics: React.FC<{
  ...
  <div className={cn('bg-black/50 rounded p-2 text-xs space-y-1', className)}>
    <div className="text-green-400 font-medium">Performance Metrics</div>
    <div className="grid grid-cols-2 gap-2 text-gray-300">
      <div>Latency: <span className="text-blue-400">{metrics.latency.toFixed(2)}ms</span></div>
      <div>Render: <span className="text-purple-400">{metrics.renderTime.toFixed(2)}ms</span></div>
      <div>Rate: <span className="text-yellow-400">{metrics.messagesPerSecond.toFixed(1)}/s</span></div>
      <div>Total: <span className="text-cyan-400">{metrics.totalMessages}</span></div>
      <div>Pool Lat: <span className="text-green-400">{wsMetrics.poolAverageLatency?.toFixed(2)}ms</span></div>
      <div>Conns: <span className="text-orange-400">{wsMetrics.connectionCount}</span></div>
    </div>
    {metrics.errors > 0 && (
...
export const ZeroLatencyTranscription: React.FC<ZeroLatencyTranscriptionProps> = ({
  ...
  const { transcription, performance, isPending, updateTranscription } = useZeroLatencyTranscription(websocketManager)
  ...
  return (
    <div className={cn('zero-latency-transcription relative', className)}>
      ...
      <div className={cn('min-h-[200px] relative', {
      ...
      {performance.latency > 0 && (
        <div className="mt-2 flex items-center justify-between text-xs text-gray-400">
          <div>
            Last update: {performance.latency.toFixed(2)}ms ago
          </div>
          <div className={cn('font-medium', {
            ...
            {performance.latency < 50 ? 'Ultra Fast' : 
             performance.latency < 100 ? 'Fast' : 'Slow'}
          </div>
        </div>
      )}
    </div>
