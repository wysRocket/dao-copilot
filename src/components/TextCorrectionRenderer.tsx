...
export const TextCorrectionRenderer: React.FC<TextCorrectionRendererProps> = ({
  ...
  useEffect(() => {
    if (correctionState.activeCorrections.length > 0 && onCorrection) {
      const latestCorrection = correctionState.activeCorrections[correctionState.activeCorrections.length - 1];
  ...
  useEffect(() => {
    const completedCorrections = correctionState.activeCorrections.filter(c => c.phase === 'complete');
    ...
    while (currentIndex < textLength) {
      ...
      for (const correction of correctionState.activeCorrections) {
        if (correction.diff.position === currentIndex) {
          ...
          const customStyle = correctionStyles[correction.diff.type as keyof typeof correctionStyles];
  ...
  return (
    ...
    >
      ...
      <div className="correction-text-container">
