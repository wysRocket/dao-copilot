  ...
  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background cursor-pointer transition-all duration-200 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        window: "bg-card border border-border hover:bg-accent hover:text-accent-foreground hover:border-accent-foreground/20 active:scale-95",
        overlay: "bg-background/80 backdrop-blur-sm border border-border/50 hover:bg-background/90 hover:border-border active:scale-95",
      },
      size: {
...
const WindowButton = React.forwardRef<HTMLButtonElement, WindowButtonProps>(
  ({ 
    ...
    const effectiveSize = size || (
      effectiveWindowType === 'overlay' ? 'compact' : 
      effectiveWindowType === 'assistant' ? 'sm' : 
      'default'
    );
    
    // Auto-detect variant based on window type if not provided
    const effectiveVariant = variant || (
      effectiveWindowType === 'overlay' ? 'overlay' :
      effectiveWindowType === 'assistant' || effectiveWindowType === 'settings' ? 'window' :
      'default'
    );
    ...
    return (
      <Comp
        className={cn(windowButtonVariants({ 
