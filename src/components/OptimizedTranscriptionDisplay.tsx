...
const PerformanceStats = memo<{
  ...
  return (
    ...
    >
      ...
      <div>
        Segments: {segmentCount} | Updates: {updateCount} | WPM: {wordsPerMinute.toFixed(1)}
      </div>
    </div>
  )
})
...
export const OptimizedTranscriptionDisplay: React.FC<OptimizedTranscriptionDisplayProps> = ({
  ...
  const {state, actions, deferredSegments, getPerformanceMetrics} =
    useOptimizedTranscriptionState(stateOptions)
  ...
  return (
    ...
    >
      ...
      <CurrentSegmentDisplay segment={state.currentSegment} enableAnimations={enableAnimations} />
      ...
      <div style={{flex: 1, position: 'relative'}}>
        {displaySegments.length === 0 ? (
          ...
          >
            ...
            <div style={{fontSize: '18px', marginBottom: '8px'}}>Ready for transcription</div>
            <div style={{fontSize: '14px'}}>Start speaking to see real-time results</div>
          </div>
          ...
          <List
            ref={listRef}
            height={
              containerRef.current?.clientHeight ? containerRef.current.clientHeight - 120 : 400
            }
