...
export const NewContentIndicator: React.FC<NewContentIndicatorProps> = ({
  ...
  const getDisplayMessage = () => {
    ...
    if (newContentCount && newContentCount > 0) {
      return newContentCount === 1 ? '1 new message' : `${newContentCount} new messages`
    }
  ...
  return (
    <div
      className={`new-content-indicator new-content-indicator--${variant} new-content-indicator--${animation} ${visible ? 'new-content-indicator--visible' : ''} ${className} `.trim()}
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={e => {
    ...
    >
      <div className="new-content-indicator__content">
        <span className="new-content-indicator__message">{getDisplayMessage()}</span>
        {getIcon()}
      </div>
    </div>
  )
...
export const ScrollControls: React.FC<ScrollControlsProps> = ({
  ...
  return (
    <div
      className={`scroll-controls scroll-controls--${position} ${className} `.trim()}
      role="group"
      aria-label="Scroll controls"
    >
      {/* Scroll percentage indicator */}
      <div className="scroll-controls__indicator">
        <span className="scroll-controls__percentage">{formatPercentage()}</span>
      </div>

      {/* Auto-scroll toggle */}
      <button
        className={`scroll-controls__button scroll-controls__auto-scroll ${isAutoScrolling ? 'scroll-controls__button--active' : ''} `.trim()}
      ...
      {/* Scroll to bottom */}
      <button
        className={`scroll-controls__button scroll-controls__bottom ${hasNewContent ? 'scroll-controls__button--highlight' : ''} `.trim()}
      ...
      >
        ...
        {hasNewContent && <div className="scroll-controls__notification-dot" />}
      </button>
    </div>
  )
}
...
export const AutoScrollContainer: React.FC<AutoScrollContainerProps> = ({
  ...
  return (
    <div className={`auto-scroll-container ${className}`} style={style}>
      ...
      <div ref={containerRef} className="auto-scroll-container__content">
        {children}
      </div>
