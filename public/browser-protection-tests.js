...
function runStackOverflowProtectionTest() {
  ...
  function recursiveTranscription(depth = 0) {
    ...
    if (typeof window.EmergencyCircuitBreaker !== 'undefined') {
      ...
      if (!allowed) {
        console.error('🚨 EMERGENCY: Circuit breaker OPEN for recursiveTranscription. Blocking call.');
...
async function runWebSocketTranscriptionTest() {
  ...
  try {
    ...
    if (typeof window.transcribeAudio === 'function') {
      ...
      console.log('Available functions:', Object.keys(window).filter(key => key.includes('transcrib')));
    }
    ...
    if (error.message.includes('Protection blocked') || 
...
function runPerformanceMonitoringTest() {
  ...
  if (typeof window.EmergencyCircuitBreaker !== 'undefined') {
    ...
    const status = breaker.getProtectionStatus ? breaker.getProtectionStatus() : breaker.getEmergencyStatus();
...
function resetCircuitBreakers() {
  ...
  if (typeof window.EmergencyCircuitBreaker !== 'undefined') {
    ...
    if (typeof breaker.resetAllBreakers === 'function') {
      ...
      ['transcribeAudio', 'transcribeAudioViaWebSocket', 'performTranscription'].forEach(funcName => {
        try {
